...
> 当前按 [更改书 002](REVISION_002_BUILD_FIRST.md) 暂缓真实复评，先完成工程建设。后续采用独立 r1 协议同时运行 A/B，不将变更口径后的结果与旧评分直接比较；缺少真实与人工证据时记“证据不足”。Step 08 工程预览建设不再等待本步，正式发布仍需 Go。下文保留原任务。
